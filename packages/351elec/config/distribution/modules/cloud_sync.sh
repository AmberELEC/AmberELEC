...
if [ ! -f "/roms/gamedata/rclone/rclone.conf" ]; then
    text_viewer -w -e -m "ERROR: /roms/gamedata/rclone/rclone.conf is missing\nPlease provide the rclone.conf file.\n\nFor more information please check the WIKI at: https://github.com/351ELEC/351ELEC/wiki" -t "351ELEC Cloud Sync" -f 18
    exit 0
fi

text_viewer -y -w -m "Do you want to backup your data on the cloud?" -t "351ELEC Cloud Sync" -f 20
response=$?
...
case $response in
    ...
    21)
        ...
        text_viewer -m "Backup completed!" -t "351ELEC Cloud Sync" -f 20
        ;;
esac