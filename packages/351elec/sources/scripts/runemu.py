...
def get_standalone_emulator_command(rom: Path, platform, emulator) -> 'Sequence[Union[str, Path]]':
	if verbose:
	...
	if platform == 'pico-8':
		...
		command += [rom if arg is None else arg for arg in placeholder_args]

	jslisten_set(jslisten_exe)
	return command

def get_retroarch_command(rom: Path, platform, core, args: 'Mapping[str, str]') -> 'Sequence[Union[str, Path]]':
	if verbose:
	...
	command += ['--config', RA_TEMP_CONF, '--appendconfig', RA_APPEND_CONF, rom_path]
...
def get_retrorun_command(rom, platform, core) -> 'Sequence[Union[str, Path]]':
...
def get_mupen64plus_standalone_command(rom, video_plugin) -> 'Sequence[Union[str, Path]]':
	if verbose:
...
def main():
	...
	if get_elec_setting('maxperf', platform, rom.name) == '1':
		if verbose:
	...
	if rom.suffix == '.sh' or platform == 'shell':
		#If the ROM is a shell script then just execute it
		command = [rom]
	elif emulator == 'retroarch':
		command = get_retroarch_command(rom, platform, core, args)
	elif emulator == 'retrorun':
		command = get_retrorun_command(rom, platform, core)
	elif emulator == 'mupen64plussa':
		command = get_mupen64plus_standalone_command(rom, core)
	else:
		command = get_standalone_emulator_command(rom, platform, emulator)
